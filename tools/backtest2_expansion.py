...
EVENT_DATES = [
    (datetime(2026, 3, 4), "Warsh formally nominated by Trump"),
    (datetime(2026, 4, 8), "Iran ceasefire agreed (initial 2-week pause)"),
    (datetime(2026, 4, 21), "Warsh confirmation hearing + ceasefire extended indefinitely"),
    (datetime(2026, 4, 28), "UAE shock announcement: OPEC exit"),
    (datetime(2026, 5, 13), "Warsh confirmed as Fed Chair (54-45, most divisive vote in history)"),
    (datetime(2026, 5, 15), "Powell term ends, Warsh term begins"),
    (datetime(2026, 6, 14), "Iran war MOU announced"),
    (datetime(2026, 6, 21), "Lake Lucerne Summit - Hormuz negotiations"),
]
...
def build_historical_state(as_of_date):
    ...
    print(f"  [GDELT] Fetching geopolitical_risk...")
    geopolitical_risk = get_geopolitical_risk_historical(as_of_date)
    print(f"  [GDELT] Fetching central_bank_buying...")
...
def run_backtest():
    ...
    for i, (as_of_date, event_label) in enumerate(EVENT_DATES):
        ...
        gold_price, gold_ma50 = price_vs_ma50("GC=F", as_of_date)
        sp_price, sp_ma50 = price_vs_ma50("^GSPC", as_of_date)
        ...
        if gold_price is not None and gold_ma50 is not None:
            price_direction_correct = (predicted_status == "ON" and gold_price > gold_ma50) or \
                                       (predicted_status == "OFF" and gold_price <= gold_ma50)

        regime_correct = None
        if regime_truth in ("SAFE_HAVEN_CONFIRMED", "SAFE_HAVEN_REJECTED"):
            regime_correct = (predicted_status == "ON" and regime_truth == "SAFE_HAVEN_CONFIRMED") or \
                              (predicted_status == "OFF" and regime_truth == "SAFE_HAVEN_REJECTED")
    ...
    csv_path = "backtest2_expansion_results.csv"
    ...
    print("\n" + "="*50)
    print("BACKTEST 2 EXPANSION SUMMARY (8 new genuine events, GDELT-based)")
    print(f"Total dates tested: {len(EVENT_DATES)}")
    ...
    print("="*50)
    print("\nSaved to backtest2_expansion_results.csv")
